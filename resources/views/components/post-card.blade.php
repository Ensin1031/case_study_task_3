...
@if ($post)
    <div class="p-4" style="width: 100%;border-radius: .5rem;box-shadow: 1px 1px 4px rgba(0, 0, 0, 0.5), -1px -1px 4px rgba(255, 255, 255, 0.1);">
        <div class="flex" style="gap: 1rem;">
            <div style="width: 15rem;display: -webkit-box;-webkit-line-clamp: 1;-webkit-box-orient: vertical;overflow: hidden;">
                <x-user-card :user="$post->user" :need_row_prefix="false" :need_shadow="false" :id_prefix="$post->id"  :need_padding="false" :redirect_to="$redirect_to" :query_parameters="$query_parameters"/>
            </div>
            <div class="flex justify-between" style="gap: 1rem;width: 100%;">
                <div style="width: 100%;">
                    <div class="flex items-center justify-between mb-4" style="height: 2rem;">
                        <h2 class="text-lg font-medium text-gray-900">{{ $post->title }}</h2>
                        @if ($can_edit)
                            @if ($post->is_public === 1)
                                <div class="ms-2 text-sm" style="padding: .1rem .5rem;border-radius: 7px;background-color: green;color: white;">Публичный</div>
                            @else
                                <div class="ms-2 text-sm" style="padding: .1rem .5rem;border-radius: 7px;background-color: blue;color: white;">Приватный</div>
                            @endif
                        @endif
                    </div>
